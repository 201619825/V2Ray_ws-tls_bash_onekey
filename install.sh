...
web_camouflage() {
    ...
    git clone https://github.com/201619825/3DCEList.git
    judge "web 站点伪装"
}
v2ray_install() {
...
domain_check() {
    read -rp "请输入你的域名信息(eg:www.201619825.com):" domain
...
v2ray_conf_add_tls() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://raw.githubusercontent.com/201619825/V2Ray_ws-tls_bash_onekey/${github_branch}/tls/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
v2ray_conf_add_h2() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://raw.githubusercontent.com/201619825/V2Ray_ws-tls_bash_onekey/${github_branch}/http2/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
#}
acme_cron_update() {
    wget -N -P /usr/bin --no-check-certificate "https://raw.githubusercontent.com/201619825/V2Ray_ws-tls_bash_onekey/dev/ssl_update.sh"
    if [[ "${ID}" == "centos" ]]; then
...
update_sh() {
    ol_version=$(curl -L -s https://raw.githubusercontent.com/201619825/V2Ray_ws-tls_bash_onekey/${github_branch}/install.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
    ...
    if [[ "$shell_version" < "$(sort -rV $version_cmp | head -1)" ]]; then
        ...
        [yY][eE][sS] | [yY])
            wget -N --no-check-certificate https://raw.githubusercontent.com/201619825/V2Ray_ws-tls_bash_onekey/${github_branch}/install.sh
...
menu() {
    ...
    echo -e "\t---authored by 201619825---"
    echo -e "\thttps://github.com/201619825\n"
